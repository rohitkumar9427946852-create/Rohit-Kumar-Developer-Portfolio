...
function Navigation({ active, open, onToggle }: { active: string; open: boolean; onToggle: () => void }) {
  return (
    <header className="fixed left-1/2 top-4 z-30 w-[min(1180px,calc(100%_-_2rem))] -translate-x-1/2 rounded-2xl border border-white/10 bg-[hsl(228_44%_9%/.78)] shadow-[0_18px_50px_-24px_hsl(228_57%_2%/.8)] backdrop-blur-2xl">
      <div className="flex h-[64px] items-center justify-between px-3 sm:px-4">
        <Brand />
        <nav className={`${open ? 'absolute left-0 right-0 top-[72px] flex' : 'hidden'} rounded-2xl border border-white/10 bg-[hsl(228_42%_9%/.98)] p-2 shadow-2xl shadow-black/30 lg:static lg:flex lg:items-center lg:gap-0.5 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none`} aria-label="Primary navigation">
          {portfolio.nav.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              onClick={onToggle}
              className={`focus-ring flex items-center gap-1.5 rounded-lg px-2.5 py-2 font-mono text-[10px] uppercase tracking-[0.1em] transition-colors ${active === item.id ? 'bg-primary/10 text-primary' : 'text-slate-400 hover:bg-white/[0.04] hover:text-slate-100'}`}
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="#contact" className="focus-ring hidden items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-3 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.1em] text-primary transition hover:bg-primary/20 lg:flex" data-testid="link-nav-connect">
            <Mail size={14} /> Connect
          </a>
          <div className="hidden lg:block"><ResumeButton /></div>
          <button type="button" className="focus-ring grid size-10 place-items-center rounded-lg border border-white/10 text-slate-300 lg:hidden" onClick={onToggle} aria-label={open ? 'Close menu' : 'Open menu'} data-testid="button-menu">
  ...
  return (
    <div className="mb-10 flex flex-col gap-4 md:mb-14 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h2 className="heading-gradient font-display text-3xl font-semibold tracking-[-0.05em] text-balance sm:text-4xl">{title}</h2>
      </div>
      {copy && <p className="max-w-md text-sm leading-6 text-slate-400 md:text-right">{copy}</p>}
    </div>
  );
}
...
function Hero() {
  return (
    <section id="home" className="relative flex min-h-[clamp(640px,calc(100svh_-_1rem),700px)] items-center overflow-hidden pt-28 lg:pt-24">
      ...
      <div className="section-wrap relative grid items-center gap-8 py-8 sm:gap-10 sm:py-10 lg:grid-cols-[1.08fr_.92fr] lg:gap-10 lg:py-12 xl:gap-14 xl:py-16">
        <div className="reveal">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/[0.06] px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-emerald-300">
          ...
          <h1 className="max-w-3xl font-display text-[clamp(2.75rem,12vw,4.5rem)] font-semibold leading-[.92] tracking-[-0.075em] text-slate-100 sm:text-[clamp(3rem,7vw,5.5rem)] lg:text-[clamp(2.75rem,6vw,5.6rem)] xl:text-[clamp(3.15rem,8vw,6.8rem)]">
            Building with curiosity.<br /><span className="text-primary">Shipping with intent.</span>
          </h1>
            <p className="mt-5 max-w-xl text-[15px] leading-7 text-slate-400 sm:text-base">{portfolio.intro}</p>
            <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.14em] text-primary/80">{portfolio.role}</p>
          ...
          <div className="mt-6 flex items-center gap-4 font-mono text-[10px] uppercase tracking-[0.16em] text-slate-600">
...
function About() {
  return <section id="about" className="section-wrap scroll-mt-24 py-24 md:py-32"><SectionHeading index="01" eyebrow="The point of view" title="Curious by default. Serious about the craft." copy="A clear view of what I’m learning, why I’m building it, and where I want to take it next." /><div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr]"><div className="relative min-h-[290px] overflow-hidden rounded-2xl border border-white/10 bg-[#0b111d] p-6"><div className="absolute -right-14 -top-14 size-48 rounded-full border border-primary/20" /><div className="absolute -right-5 top-[-2rem] size-32 rounded-full border border-violet-300/20" /><div className="relative flex h-full flex-col justify-between"><div><span className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">/about/rohit</span><div className="mt-8 font-display text-6xl font-semibold tracking-[-0.08em] text-slate-200">RK<span className="text-primary">.</span></div></div><div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.15em] text-slate-500"><span>Curious by default</span><span>Lucknow → anywhere</span></div></div></div><div className="flex flex-col justify-center gap-5 text-[15px] leading-7 text-slate-400">{portfolio.bio.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<div className="mt-2 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-primary/15 bg-primary/[0.04] p-4"><Code2 size={18} className="mb-3 text-primary" /><div className="text-sm font-semibold text-slate-200">Build things people can use</div><div className="mt-1 text-xs leading-5 text-slate-500">Readable UI, intentional decisions, useful details.</div></div><div className="rounded-xl border border-violet-300/15 bg-violet-300/[0.04] p-4"><BrainCircuit size={18} className="mb-3 text-violet-300" /><div className="text-sm font-semibold text-slate-200">Keep learning in public</div><div className="mt-1 text-xs leading-5 text-slate-500">From fundamentals to systems, one concept at a time.</div></div></div></div></div></section>;
}
